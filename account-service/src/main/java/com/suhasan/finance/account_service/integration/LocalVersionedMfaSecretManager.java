...
@Component
@SuppressWarnings("PMD.HardCodedCryptoKey") // "AES" selects an algorithm; key material is derived from configuration.
public class LocalVersionedMfaSecretManager implements MfaSecretManager {
    ...
    public LocalVersionedMfaSecretManager(
            @Value("${integration.mfa.kms.provider:local-key}") final String provider,
            @Value("${integration.mfa.kms.active-key-id:local-v1}") final String activeKeyId,
            @Value("${security.mfa.encryption-key:}") final String activeSecret,
            @Value("${integration.mfa.kms.previous-keys:}") final String previousKeys) {
        this.provider = provider == null ? "unconfigured" : provider.trim();
        this.activeKeyId = requireKeyId(activeKeyId);
        final Map<String, SecretKeySpec> configured = new LinkedHashMap<>();
        ...
        if (previousKeys != null && !previousKeys.isBlank()) {
            for (final String entry : previousKeys.split(";")) {
                final String[] pair = entry.trim().split("=", 2);
    ...
    @Override
    public Ciphertext encrypt(final String plaintext) {
        try {
    ...
    @Override
    public String decrypt(final String ciphertext, final String keyId) {
        try {
            String payload = ciphertext;
            final String resolved = keyId == null || keyId.isBlank() ? "legacy" : keyId;
            if (ciphertext.startsWith("kms:v1:")) {
                final String[] parts = ciphertext.split(":", 4);
                if (parts.length != 4 || !parts[2].equals(resolved)) {
                    throw new IllegalStateException("MFA ciphertext key version mismatch");
                }
                payload = parts[3];
            }
            final ByteBuffer buffer = ByteBuffer.wrap(Base64.getDecoder().decode(payload));
            final byte[] iv = new byte[12];
            buffer.get(iv);
    ...
    public Health health() {
        final boolean configured = keys.containsKey(activeKeyId);
        final boolean local = provider.startsWith("local");
    ...
    private SecretKeySpec derive(final String secret) {
        try {
    ...
    private SecretKeySpec require(final String keyId) {
        final SecretKeySpec key = keys.get(keyId);
        if (key == null) throw new IllegalStateException("MFA KMS key version is unavailable: " + keyId);
        return key;
    }
    private String requireKeyId(final String value) {
