...
public class SpendingLimitService {
 ...
 @Transactional(readOnly=true) public List<SpendingLimitDtos.LimitResponse> list(final String user){ return accounts.findAll().stream().filter(a->user.equals(a.getOwnerId())).map(a->view(a,limits.findById(a.getId()).orElse(null))).toList(); }
 ...
 @Transactional public SpendingLimitDtos.ReserveResponse reserve(final Long accountId,final SpendingLimitDtos.ReserveRequest r){
  ...
  if(reservations.existsByAccountIdAndOperationTypeAndIdempotencyKey(accountId,type,r.idempotencyKey())) return new SpendingLimitDtos.ReserveResponse(true,true,a.getCurrency(),limit,used,limit.subtract(used).max(BigDecimal.ZERO),null);
  final BigDecimal projected=used.add(r.amount()); if(projected.compareTo(limit)>0){ audit(accountId,r.userId(),"LIMIT_REJECTED",type,r.amount(),limit,used,"Daily spending limit exceeded"); notify(r.userId(),accountId,"Operation rejected","A "+type.toLowerCase(Locale.ROOT)+" was rejected because it exceeds your daily limit.","limit-reject-"+accountId+"-"+type+"-"+r.idempotencyKey()); return new SpendingLimitDtos.ReserveResponse(false,false,a.getCurrency(),limit,used,limit.subtract(used).max(BigDecimal.ZERO),"Daily spending limit exceeded"); }
  reservations.save(SpendingLimitReservation.builder().accountId(accountId).operationType(type).amount(r.amount()).usageDate(LocalDate.now()).idempotencyKey(r.idempotencyKey()).createdAt(LocalDateTime.now()).build()); audit(accountId,r.userId(),projected.compareTo(limit.multiply(new BigDecimal("0.80")))>=0?"LIMIT_APPROACHING":"LIMIT_ENFORCED",type,r.amount(),limit,projected,"Daily usage reserved before debit processing"); if(projected.compareTo(limit.multiply(new BigDecimal("0.80")))>=0) notify(r.userId(),accountId,"Approaching daily limit","Your "+type.toLowerCase(Locale.ROOT)+" usage has reached at least 80% of today's limit.","limit-near-"+accountId+"-"+type+"-"+LocalDate.now()); return new SpendingLimitDtos.ReserveResponse(true,false,a.getCurrency(),limit,projected,limit.subtract(projected),null);
 }
 ...
 private SpendingLimitDtos.LimitResponse view(final Account a,final AccountSpendingLimit l){ final Long id=a.getId(); final BigDecimal t=l==null?new BigDecimal("10000.00"):l.getTransferDailyLimit(),w=l==null?new BigDecimal("2000.00"):l.getWithdrawalDailyLimit(); return new SpendingLimitDtos.LimitResponse(id,a.getCurrency(),t,w,reservations.used(id,"TRANSFER",LocalDate.now()),reservations.used(id,"WITHDRAWAL",LocalDate.now()),l==null?null:l.getPendingTransferDailyLimit(),l==null?null:l.getPendingWithdrawalDailyLimit(),l==null?null:l.getPendingEffectiveAt()); }
