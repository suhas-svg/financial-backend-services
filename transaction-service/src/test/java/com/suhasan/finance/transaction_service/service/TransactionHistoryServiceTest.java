...
class TransactionHistoryServiceTest {
        ...
        void testSearchTransactions() {
                ...
                // Act — searchTransactions now delegates to findTransactionsWithFilters (M2
                // fix)
                Page<Transaction> scoped = new PageImpl<>(List.of(sampleTransaction));
                when(transactionRepository.findTransactionsWithFilters(
                                eq("account-1"), eq(TransactionType.TRANSFER), eq(TransactionStatus.COMPLETED),
                                any(), any(), eq(BigDecimal.valueOf(50.00)), eq(BigDecimal.valueOf(500.00)),
                                any(), any(), eq("test-user"), eq(pageable)))
                                .thenReturn(scoped);
        ...
        void testGetAccountTransactionStats() {
                ...
                TransactionStatsResponse result = transactionService.getAccountTransactionStats(accountId, startDate,
                                endDate);
