...
from mcp.types import Tool, TextContent, InitializeRequest, InitializeResult, ServerCapabilities
...
class FinancialMCPServer:
    ...
    def __init__(self, settings: Optional[Settings] = None):
        ...
        if not self.settings.account_service_url:
            raise ValueError("Account service URL is required")
        if not self.settings.transaction_service_url:
            raise ValueError("Transaction service URL is required")
        if not self.settings.jwt_secret or self.settings.jwt_secret == "your-secret-key":
            raise ValueError("A non-default JWT secret is required")
        self.app = FastMCP("Financial Services MCP", host=self.settings.host, port=self.settings.port)
    ...
    async def shutdown(self) -> None:
        ...
        # Close downstream HTTP clients.
        if hasattr(self, 'account_client'):
            await self.account_client.close()
        if hasattr(self, 'transaction_client'):
    ...
    async def __aenter__(self) -> "FinancialMCPServer":
        """Enter a managed server lifecycle."""
        return self

    async def __aexit__(self, exc_type, exc_value, traceback) -> None:
        """Release server resources on context exit."""
        await self.shutdown()
