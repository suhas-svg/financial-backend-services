...
export function AccountsPage() {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[420px_1fr]">
      ...
      <Panel title={editing ? `Edit account #${editing.id}` : "Create account"}>
      ...
      <Panel
        title="Accounts"
        action={
          <Select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} className="w-36">
      ...
      >
        {accounts.data?.content.length ? (
                  ...
                  <tr key={account.id} className="border-b border-line last:border-0">
                    ...
                    <td className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" onClick={() => setSelected(account)} aria-label={`View account ${account.id}`}>
                          View
                        </Button>
          ...
          <EmptyState title="No accounts found" detail="Try another filter or create a new account." />
        )}
      </Panel>
    </div>
  );
